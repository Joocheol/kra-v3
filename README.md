...
- 평균 차이 `trifecta_uniform - win_harville`: **-0.00098517**
  - 날짜 cluster bootstrap 95%: **[-0.00136726, -0.00059228]**
- 2·3착 축을 뒤집으면 평균 Brier가 **0.9927140**으로 악화
  - `swapped_23 - uniform`: **0.00108387**
  - 날짜 cluster bootstrap 95%: **[0.00057216, 0.00160883]**
...
따라서 이 주지표에서는 삼쌍승 가격이 단승 가격을 Harville로 순차확장한 기준선보다
실제 상위 세 착순과 더 잘 정렬된다. 2·3착 축 반전이 실제 착순에서도
악화된다는 결과는 페이지 방향성에 대한 외부 반증검사이기도 하다. 다만 이는
정규화한 시장가격을 확률예측처럼 채점한 결과이며, 시장효율성·객관적 확률의
정확성·인과효과를 뜻하지 않는다.

### Claude 2차 검토 후 강건성 분석

착순평가 코드가 최초 수치 보고서보다 먼저 커밋된 순서는 확인되지만, 외부
사전등록을 입증하지는 않는다. 따라서 주지표 외 추가 비교는 사후 강건성
분석으로 구분한다. 2022--2024 착순으로만 2·3착 할인모수를 각각 적합한
dual-discounted Harville(`lambda2=0.800`, `lambda3=0.675`)을 2025년에
고정 적용했다.

- 2025 삼쌍승 균등완성 Brier: **0.9916302**
- 2025 dual-discounted Harville Brier: **0.9922367**
- 평균 차이: **-0.00060654**
  - 날짜 cluster bootstrap 95%: **[-0.00098558, -0.00023534]**
- 모든 기준선을 같은 2,312경주에 맞추면 ordinary Harville →
  dual-discounted → 쌍승+단승 → 삼복승+쌍승 차이는
  **-0.00084894 → -0.00056461 → -0.00041412 → -0.00026665**로 감소
- 가장 강한 삼복승+쌍승 기준선과의 차이는 **-0.00026665**
  (95% **[-0.00049263, -0.00003313]**). 0에 가까운 구간 끝점은 미검열
  표시배당 반올림의 보수적 평균 Brier 외부변동 상한 **0.00013152**보다 작아,
  전체 반올림 식별불확실성 아래에서는 우위의 부호가 확정되지 않음
- 출전두수별 dual-discounted 비교는 10--11두에서만 구간 전체가 음수다.
  7--9두와 12--14두는 0을 포함하고, 15--16두는 점추정치가 반대 방향
  (+0.00336626)이지만 10경주·9개 날짜뿐이라 CI를 제시하지 않음
- 2022--2024년 7,245경주 retrospective replication:
  삼쌍승 - 일반 Harville **-0.00085216**
  (95% **[-0.00109107, -0.00061252]**)
- `residual_mid`와 한 미검열 정수완성을 고정한 조건부 상한셀 배분 Brier
  경계: **0.9916300--0.9916306**; 미검열 표시배당 반올림 여유의 보수적
  평균 Brier 외부변동 상한은 **0.00013152**

핵심적으로 이 결과는 상한 복원 자체의 시간외 검증이 아니다. 2025년 실현
상한상태가 8/2,426건에 불과하고, 상한 배분 경계 폭은 비교모형 간 점수차보다
약 1,000배 작다. 따라서 이 단계는 대부분 미검열된 삼쌍승 가격표와 풀 정보가
착순에 얼마나 정렬되는지 평가한다.

상한 잔여질량 `R/T`에 대한 직접 Poisson-binomial 진단에서 관측 8건에 비해
기대 capped 적중수는 하한·중간·상한 시나리오별 12.204 / 13.192 / 14.181건이며,
95% 예측구간은 각각 [6,19] / [7,21] / [7,22]다. 세 시나리오 모두 관측 8건을
배제하지 않는다(양측 tail p=0.2808 / 0.1795 / 0.1110).

동일한 모수무관 검정을 2022--2025 전체 9,671경주에 적용하면 관측 43건에
비해 기대값은 55.282 / 59.363 / 63.444건이다. residual_mid와 residual_max는
독립 Bernoulli 가정 아래 각각 p=0.0311, p=0.0080으로 관측 capped 적중수를
과대예측한다. 같은 날짜 경주 간 의존성을 반영하지 않은 민감도 진단이므로
확정적 기각으로 해석하지 않으며, residual_mid는 식별구간의 중립적 관례로만 둔다.

실제 적중조합이 `9999.9`였던 8경주는 지급식 마권 수가 모두 점식별됐지만,
균등배정의 절대 상대오차 중앙값은 24.4%, 최댓값은 160.8%다. 지급표가 존재하는
표본은 기계적으로 `n>=1`이므로 `n=0` 식별에는 아무 정보도 주지 않으며,
이 대조 결과를 회계 경계에 되먹이지 않는다. 자세한 결과는
`findings/outcome_robustness.md`와
`데이터/outcome_robustness.csv.gz`에 동결한다.
