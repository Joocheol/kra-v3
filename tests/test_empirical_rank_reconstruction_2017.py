...
def aggregate_profile(rows):
    a=np.atleast_2d(np.asarray(rows,dtype=float))
...
def rank_scores(profile,J,k):
    start=J-k
    return np.asarray([math.exp(float(profile[min(BINS-1,int(((i+0.5)/J)*BINS))])) for i in range(start,J)],float)


def allocate(profile,J,k,total,U=None):
    if k == 0: return np.empty(0,dtype=np.int64)
    scores=rank_scores(profile,J,k); scores=scores/scores.sum()
    target=total*scores
    ...
    def test_rank_profile(self):
        ...
        for frac in (0.20,0.40,0.60):
            ...
            for _,truth_all in valid:
                ...
                p=allocate(profile,J,k,total); u=uniform_alloc(k,total)
        ...
        for r in strict_capped:
            ...
            lo=int(r['feasible_residual_min']); hi=int(r['feasible_residual_max']); R=(lo+hi)//2
            ...
            if sims:
                A=np.vstack(sims); q05=np.quantile(A,0.05,axis=0); q95=np.quantile(A,0.95,axis=0)
                spread.append(float(np.mean(q95-q05))); d=len(hashes)
